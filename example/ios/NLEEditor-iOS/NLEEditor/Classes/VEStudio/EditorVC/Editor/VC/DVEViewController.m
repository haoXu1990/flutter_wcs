...
- (void)initAddMethod
{
    ...
    [[self.addButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        [resourcePicker pickResourcesWithCompletion:^(NSArray<id<DVEResourcePickerModel>> * _Nonnull resources, NSError * _Nullable error, BOOL cancel) {
            @strongify(self);
            [DVEAutoInline(self.vcContext.serviceProvider, DVECoreImportServiceProtocol) addResources:resources];
...
- (void)initRACObserve
{
    ...
    [[RACObserve(self.vcContext.mediaContext, selectMainVideoSegment).distinctUntilChanged skip:1] subscribeNext:^(NLETrackSlot_OC *  _Nullable selectedSegment) {
        ...
        if (selectedSegment) {
            ...
            //选中时，若当前有关键帧渐变参数，需要更新canvas border
            [self.videoPreview refreshCanvasBorderForKeyFrameIfNeed];
            //选中时，更新蒙版参数
            [DVEAutoInline(self.vcContext.serviceProvider, DVECoreMaskProtocol) refreshMaskForKeyFrameIfNeed:selectedSegment];
        } else {
    ...
    [[[RACObserve(self.vcContext.mediaContext, selectTextSlot) distinctUntilChanged] skip:1] subscribeNext:^(NLETrackSlot_OC *_Nullable slot) {
        ...
        if (slot) {
            ...
            if (isTextSticker) {
                ...
                [manager showComponentType:DVEBarComponentTypeTextTemplate groupTpye:DVEBarSubComponentGroupEdit];
            } else {
...
- (void)dealRedLine
{
    [self.stickerEditAdatper isShowBox];
