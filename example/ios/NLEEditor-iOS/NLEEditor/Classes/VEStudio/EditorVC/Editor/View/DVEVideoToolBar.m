...
- (UIButton *)undoButton
{
    if (!_undoButton) {
        ...
        [[_undoButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            @strongify(self);
            [DVEAutoInline(self.vcContext.serviceProvider, DVECoreActionServiceProtocol) excuteUndo];
        }];
    }
...
- (UIButton *)redoButton
{
    if (!_redoButton) {
        ...
        [[_redoButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            @strongify(self);
            [DVEAutoInline(self.vcContext.serviceProvider, DVECoreActionServiceProtocol) excuteRedo];
        }];
    }
...
-(void)updateKeyframeButtonStatues
{
    ...
    if(CMTimeRangeContainsTime(slot.nle_targetTimeRange, time)){
        ...
        if([self.keyFrameEditor hasKeyframe:slot forTime:time]){
...
-(void)actionKeyframe
{
    ...
    if(self.keyframeButton.selected){
        if([self.keyFrameEditor addOrUpdateKeyFrame:slot forTime:time commit:YES]){
            [self showRemoveKeyframe];
        }
    }else{
        if([self.keyFrameEditor removeKeyframe:slot forTime:time]){
            [self showAddKeyframe];
        }
    }
}
