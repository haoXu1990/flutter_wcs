...
@interface DVEStickerEditAdpter () <DVETransformEditViewDelegate,DVECoreActionNotifyProtocol>
...
- (DVETransformEditViewConfig *)boxconfig
{
    if (!_boxconfig) {
        ...
        boxconfig.topRight = [[DVEEditBoxCornerInfo alloc] initWithImage:@"stickerIcMirrorN".dve_toImage  highlightImage:@"textIcCloseP".dve_toImage type:DVECornerMirror];
        boxconfig.bottomLeft = [[DVEEditBoxCornerInfo alloc] initWithImage:@"textIcCopyN".dve_toImage  highlightImage:@"textIcCloseP".dve_toImage type:DVECornerCopy];
        boxconfig.bottomRight = [[DVEEditBoxCornerInfo alloc] initWithImage:@"textIcRorateN".dve_toImage  highlightImage:@"textIcCloseP".dve_toImage type:DVECornerPinch];
...
- (void)showInPreview:(UIView *)view  withType:(VEVCStickerEditType)type
{
    ...
    switch (type) {
        ...
        case VEVCStickerEditTypeTextTemplate:
        {
            ...
            [self refreshItems:self.stickerEditor.textSlots];
        }break;
        case VEVCStickerEditTypeSticker:{
...
- (void)isShowBox {
    ...
    if(slot && NLE_CMTimeRangeContain(slot.nle_targetTimeRange, current)) {
        [self.editView activeEditItem:slot.nle_nodeId];
    } else {
...
- (void)changeSelectTextSlot:(NSString *)segmentId {
    if (segmentId) {
        ...
        self.textBar.segmentId = segmentId;
    } else {
...
- (void)updateBorderElementsSizeWithItem:(DVEEditItem *)item {
    ...
    if (slot.segment.getType != NLEResourceTypeTextTemplate || !textInfo) {
...
- (DVEEditItem *)createEditItemForSlot:(NLETrackSlot_OC *)slot {
    ...
    CGFloat offsetX = visibleSize.width * 0.5 * (1 + x);
    CGFloat offsetY = visibleSize.height * 0.5 * (1 - y);
   // note(caishaowu): 也可以直接使用-[NLEInterface_OC stickerBoxWithSlot:]
...
- (void)transformView:(DVETransformEditView *)editView didTriggerAction:(DVEEditCornerType)type item:(DVEEditItem *)item {
    if (type == DVECornrDelete) {
        ...
        if (_curType == VEVCStickerEditTypeTextTemplate) {
            NSString *newSegId = [self.textTemplateEditor copyTextTemplateWithIsCommit:YES];
            // 添加编辑框
            [self addEditBoxForSticker:newSegId isText:YES];
        } else {
            NLETrackSlot_OC *newSlot = [self.slotEditor copyForSlot:item.resourceId needCommit:YES];
            NSInteger veStickerId = [self.nle stickerIdForSlot:newSlot.nle_nodeId];
            // the「isText」param is not used in this function, hard code for the time being.
            [self addEditBoxForSticker:newSlot.nle_nodeId isText:YES];
        }
