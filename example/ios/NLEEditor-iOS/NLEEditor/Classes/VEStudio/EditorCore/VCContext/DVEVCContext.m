                 ...
                 injectService:(id<DVEVCContextExternalInjectProtocol>)injectService
{
    ...
    self.mediaContext = [self createMediaContext];
    [DVEAutoInline(self.serviceProvider, DVECoreImportServiceProtocol) addNLEMainVideoWithResources:resources];
    [self.mediaContext updateMappingTimelineVideoSlot];
    return self;
}
...
#pragma mark - NLEEditor_iOSListenerProtocol

- (void)didDoneChanged:(NLEEditorDoneType)doneType msg:(NSString *)msg
{
    if(doneType == NLEEditorDoneTypeDONE) {
        return;
    }
    else if (doneType == NLEEditorDoneTypeREDO) {
