                                   ...
                                   slot:(NLETrackSlot_OC *)slot;
{
    ...
    NSString *fontPath = @"";
    if (parm.font.sourcePath.length > 0) {
        fontPath = parm.font.sourcePath;
    } else if (style.font.resourceFile.length > 0) {
        NSString *path = @"font/";
        if (![style.font.resourceFile.lastPathComponent isEqualToString:@"zh-hans.ttf"]) {
            NSString *fontname = [style.font.resourceFile.lastPathComponent stringByDeletingPathExtension];
            path = [path stringByAppendingFormat:@"%@/",fontname];
        }
        
        path = [path stringByAppendingString:style.font.resourceFile.lastPathComponent];
        fontPath = [path pathInBundle:@"Text"];
    } else {
        fontPath = @"";
    }
    NSString *text = nil;
...
//    }
    ...
    // 阴影
    params[@"shadow"] = @(YES);
    params[@"shadowSmoothing"] = @(parm.shadowSmoothing/18.0);
