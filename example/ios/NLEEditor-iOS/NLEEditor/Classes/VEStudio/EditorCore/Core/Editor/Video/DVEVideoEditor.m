...
@property (nonatomic, assign) BOOL canKeyFrameCallback;
...
- (instancetype)initWithContext:(DVEVCContext *)context
{
    if(self = [super init]) {
        ...
        [[[RACObserve(self.vcContext.playerService, currentPlayerTime) distinctUntilChanged] deliverOnMainThread] subscribeNext:^(id  _Nullable x) {
...
- (void)videoFreezeForSlot:(NLETrackSlot_OC *)slot isMain:(BOOL)main
{
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        NSArray<NLETrackSlot_OC *> *keyFrames = [slot getSortKeyframe];
        //如果当前slot有关键帧，取前一个slot最后一个关键帧的数据
        if (keyFrames.count > 0) {
            trackSlot = [[keyFrames lastObject] deepClone:YES];
        } else {
            trackSlot = [[NLETrackSlot_OC alloc] init];
            [trackSlot setScale:main ? 1.0 : 0.7];
        }
...
- (void)handleVideoReverse:(NLETrackSlot_OC *)slot isMain:(BOOL)main {
    ...
    void(^handleReverse)(AVURLAsset *,AVURLAsset *,BOOL) = ^(AVURLAsset *before, AVURLAsset *after, BOOL isReallyReverse) {
        @strongify(self);
        [self reverseSlot:slot withAsset:after forMainTrack:main isReallyReverse:isReallyReverse autoCommit:NO];
        if (isReallyReverse) {
...
- (BOOL)containsVideoSlot:(NLETrackSlot_OC *)slot {
    self.nleEditor.nleModel;
    return YES;
}
         ...
         autoCommit:(BOOL)autoCommit
{
    ...
    [slot setRewindAndAdjustKeyframe:isReallyReverse];
    if (forMainTrack) {
          ...
          shouldKeepTone:(BOOL)shouldKeepTone
{
    ...
    if([self.keyFrameEditor hasKeyframe:slot]){
        [slot setSpeedAndAdjustKeyframe:speed];
    }else{
            ...
            shouldCommit:(BOOL)commit
{
    ...
    if([self.keyFrameEditor hasKeyframe:slot]){
        [slot setSegCurveSpeedPointAndAdjustKeyframe:points];
    }else{
                ...
                commit:(BOOL)commit
{
    ...
    if ([slot.segment isKindOfClass:NLESegmentAudio_OC.class]) {
        // 重新应用变速点
        NSArray *oriPoints = [(NLESegmentAudio_OC *)slot.segment getSegCurvePoints];
        [self updateVideo:slot curveSpeedPoint:oriPoints shouldCommit:NO];
        
        NSArray *newPoints = [(NLESegmentAudio_OC *)newSlot.segment getSegCurvePoints];
        [self updateVideo:newSlot curveSpeedPoint:newPoints shouldCommit:NO];
    }
    //拆分后所有选中的视频slot置空
...
- (void)changeSlot:(NLETrackSlot_OC *)slot volume:(CGFloat)volume
{
    ...
    if ([self.keyFrameEditor hasKeyframe:slot]) {
        ...
        [self.keyFrameEditor addOrUpdateKeyFrame:slot
                                         forTime:time
                                          commit:NO];
        //当前处于关键帧增加或更新状态，不应该响应关键帧回调给UI
        self.canKeyFrameCallback = NO;
    }
                ...
                keyFrameInfo:(NLEAllKeyFrameInfo *)info
{
    ...
    NSUInteger pts = self.vcContext.playerService.currentPlayerTime * USEC_PER_SEC;
    CGFloat volume = [self.nle audioVolumeKeyFrameInfoWithPTS:pts slot:slot];
