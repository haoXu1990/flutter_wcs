...
@interface DVETextBar ()<SGPageTitleViewDelegate,UITextViewDelegate, DVEPickerViewDelegate, DVEAnimationViewDelegate, DVEAnimationViewDataSource>
...
- (DVETextStyleView *)styleView
{
    if (!_styleView) {
        ...
        _styleView.colorBlock = ^(DVEEffectValue * _Nonnull color, NSInteger colorType, NSDictionary * _Nonnull extraDict) {
            ...
            if (colorType == DVETextColorConfigTypeFont ||
                colorType == DVETextColorConfigTypeBackground) {
                self.curParm.alpha = color.indesty;
            }
    ...
    if (!_dismissBack) {
        ...
        [[_dismissBack rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            if (self.curParm.text.length == 0) {
                [self.slotEditor removeSlot:self.segmentId needCommit:NO isMainEdit:self.isMainEdit];
                [vc.stickerEditAdatper removeStickerBox:self.segmentId];
                ...
                [self.stickerEditor updateTextStickerWithParm:self.curParm segmentID:self.segmentId isCommit:YES isMainEdit:self.isMainEdit];
            ...
            vc.stickerEditAdatper.textBar = nil;
        }];
    }
...
//    NLETrackSlot_OC *stickerSlot = [self.nleEditor.nleModel slotOf:self.segmentId];
//    NLESegmentTextSticker_OC *segTextSticker = (NLESegmentTextSticker_OC  *)stickerSlot.segment;
//    NLEStyleText_OC *resTextStyle = segTextSticker.style; 
//    self.curParm.typeSettingKind = resTextStyle.typeSettingKind;
...
- (void)showInView:(UIView *)view animation:(BOOL)animation
{
    ...
    [_functionView setSelectedIndex:0];
    [self.textView becomeFirstResponder];
...
- (void)setVcContext:(DVEVCContext *)vcContext {
    ...
    // 从 segTextSticker 得到 json 数据后恢复
