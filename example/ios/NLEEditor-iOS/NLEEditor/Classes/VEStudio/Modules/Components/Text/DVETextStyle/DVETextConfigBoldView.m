...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        @weakify(self);
        _blodBtn = [self p_createBtnWithImgName:@"bold" clickBlock:^(UIButton *btn) {
            @strongify(self);
            self.boldWidth = btn.isSelected ? 0.008f : 0.0f;
        }];
        
        _italicBtn = [self p_createBtnWithImgName:@"italic" clickBlock:^(UIButton *btn) {
            @strongify(self);
            self.italicDegree = btn.isSelected ? 10.0f : 0.0f;
        }];
        
        _underlineBtn = [self p_createBtnWithImgName:@"underline" clickBlock:^(UIButton *btn) {
            @strongify(self);
...
- (UIButton *)p_createBtnWithImgName:(NSString *)imgName clickBlock:(void (^)(UIButton *btn))block {
    ...
    NSString *path = [[imgName stringByAppendingString:@".png"] pathInBundle:@"Text"];
