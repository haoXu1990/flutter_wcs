...
- (void)setupUI
{
    ...
    self.maskLabel.frame = CGRectMake(16, sliderY, 24, 20);
    [self addSubview:self.maskLabel];

    [self.slider removeFromSuperview];
    self.slider = [[DVEStepSlider alloc] initWithStep:1 defaultValue:0 frame:CGRectMake((self.width - sliderWidth) / 2, sliderY, sliderWidth, 20)];
...
- (BOOL)initSelectMask
{
    ...
    if (findMask) {
        ...
        if (index > 0) {
            ...
            /*
             因为调整了DVEKeyFrameEditor的addOrUpdateKeyFrame逻辑，不再单纯直接替换当前slot快照，所以这块逻辑暂时先屏蔽观察。如果没问题可以直接删了
             
            NLETrackSlot_OC* keyframe = [self.keyframeEditor keyframeInSlot:slot forTime:self.vcContext.mediaContext.currentTime];
            if (keyframe != nil) {
                ///优先检测当前时间是否存在关键帧
                ///当前时间在关键帧范围内的时候，VE返回的蒙版实时数据会跟当前关键帧数据存在一定偏差
                ///如果使用偏差数据调节滑杆，会出现编辑边框不停偏移
                findMask = [keyframe getMask].firstObject;
                maskSegment = findMask.segmentMask;
            }///如果都不存在关键帧就使用Model中的数据进行初始化
             */
            self.vcConfigModel.width = maskSegment.width;
            self.vcConfigModel.height = maskSegment.height;
            self.vcConfigModel.feather = maskSegment.feather;
