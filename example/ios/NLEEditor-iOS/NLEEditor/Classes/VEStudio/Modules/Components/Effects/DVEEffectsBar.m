...
///进入特效页面时播放器的时间
@property (nonatomic, assign) CMTime initPlayerTime;
...
- (void)initData
{
    ...
    self.initPlayerTime = self.vcContext.mediaContext.currentTime;
...
- (void)setCurValue:(DVEEffectValue *)curValue {
    ...
    if(curValue){
        if(_curValue){///已有特效，则做替换操作
            ...
            NSInteger endSecond = MIN((CMTimeGetSeconds(self.initPlayerTime) + 3), CMTimeGetSeconds(self.vcContext.mediaContext.duration));
            ...
            if ([config respondsToSelector:@selector(onlyUseGlobalEffect)] && [config onlyUseGlobalEffect]) {
                self.effectObjID = [self.effectEditor addGlobalNewEffectWithPath:curValue.sourcePath
                                                                            name:curValue.name
                                                                       startTime:CMTimeMake(CMTimeGetSeconds(self.initPlayerTime) * USEC_PER_SEC, USEC_PER_SEC)
                                                                         endTime:CMTimeMake(endSecond * USEC_PER_SEC, USEC_PER_SEC)
    ...
    if(!_bottomView) {
        ...
        _bottomView = [DVEEffectsBarBottomView newActionBarWithTitle:NLELocalizedString(@"ck_effect",@"特效") action:^{
            ...
            if(self.effectObjID && !self.vcContext.mediaContext.selectEffectSegment){
                ...
                [DVECustomerHUD showMessage:@"添加特效轨道成功" afterDele:1.0];
            }
        }];
    }
    return _bottomView;
