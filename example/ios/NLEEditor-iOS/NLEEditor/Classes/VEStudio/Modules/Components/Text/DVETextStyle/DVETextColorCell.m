...
- (NSDictionary<NSNumber *,DVETextColorConfigModel *> *)p_createModelDictWithColorArr:(NSArray *)colorArr {
    ...
    fontM.selectedIndex = -1;
    
    /// 空的颜色
    DVEEffectValue *value = [DVEEffectValue new];
    value.valueState = VEEffectValueStateShuntDown;
    NSString *path = [@"nothing@3x.png" pathInBundle:@"Text"];
    value.imageURL = [NSURL fileURLWithPath:path];
    ...
    switch (_colorType) {
        ...
        case DVETextColorConfigTypeBackground:
        {
            ...
            if (m.selectedIndex >= 0) {
                ...
                value.color = [tmpV.color mutableCopy];
            }
            float sliderValue = self.slider.value;
            m.sliderValue = sliderValue;
            if (m.sliderValueRange.length > 0) {
                value.indesty = m.sliderValue / m.sliderValueRange.length;
            } else {
                value.indesty = 1.0;
            }
        }
        ...
        case DVETextColorConfigTypeShadow:
        {
            ...
            dict[@"shadowOffset"] = @[@(point.x/18.0), @(point.y/18.0)];
            dict[@"shadowSmoothing"] = @(_shadowView.blurSlider.value / 100.0 * 3.0);
        }
            break;
        case DVETextColorConfigTypeArrange:
        {
