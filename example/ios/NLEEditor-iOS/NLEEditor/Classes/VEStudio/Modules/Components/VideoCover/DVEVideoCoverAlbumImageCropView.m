...
- (void)setUpLayout {
    ...
    [self.hintLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.mas_top).offset((80 - VETopMargnValue + [DVEUIHelper topBarMargn] - 10));
    }];
...
- (void)configCropPreview {
    ...
    CGSize maxSize = CGSizeMake(self.frame.size.width - 2 * 12, self.frame.size.height - 2 * 56);
    self.previewSize = DVE_aspectFitSize(self.imageView.image.size, maxSize);
    self.cropSize = self.previewSize;
    ...
    self.scrollView.contentSize = CGSizeMake(self.previewSize.width + 12, self.previewSize.height + 56);
    self.scrollView.contentOffset = CGPointMake(self.previewSize.width / 2 - maxSize.width / 2, self.previewSize.height / 2 - maxSize.height / 2);
    ...
    CGFloat cropHeight = self.cropSize.width / self.cropRatio;
    CGRect cropRect = CGRectMake(cropOrigin.x, (self.height - cropHeight) / 2, self.cropSize.width, cropHeight);
    
    if (self.previewSize.height < cropHeight) {
        self.scrollView.zoomScale = cropHeight / self.previewSize.height;
    }
...
CGSize DVE_aspectFitSize(CGSize size, CGSize maxSize) {
    ...
    if (wRatio > hRatio) {
        ...
        resultSize = CGSizeMake(maxSize.height, maxSize.height * size.width / size.height);
    }
    
    if (resultSize.width > maxSize.width || resultSize.height > maxSize.height) {
        resultSize.width = maxSize.width;
        resultSize.height = maxSize.width * (size.height / size.width);
    }
    return resultSize;
}
